...
(function () {
  ...
  if (SEO[slug] && /^\/tools\//i.test(path)) {
    ...
    if (!document.getElementById('dt-tool-guide')) {
      ...
      document.head.appendChild(style); var mountGuide = function(){ var target = document.querySelector('main') || document.body; if(target) target.appendChild(guide); }; if(document.body) mountGuide(); else document.addEventListener('DOMContentLoaded', mountGuide, {once:true});
    }
  }
})();
