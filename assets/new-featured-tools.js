document.addEventListener('DOMContentLoaded',function(){
  ...
  // The homepage declares allTools with a global lexical const. Access it here so
  // its existing search, category filters and card design remain the single source of truth.
  try{
    if(typeof allTools!=='undefined' && Array.isArray(allTools)){
      const existing=new Set(allTools.map(function(t){return t.url}));
      tools.slice().reverse().forEach(function(t){if(!existing.has(t[3]))allTools.unshift({name:t[1],desc:t[2],icon:'fas fa-star',cat:t[4],url:t[3]})});
      if(typeof renderTools==='function')renderTools();
      const grid=document.getElementById('toolsGrid');
      if(grid){const marker=document.createElement('div');marker.id='premiumToolsMarker';marker.style.cssText='grid-column:1/-1;background:#0A0A0A;color:#fff;padding:14px 18px;font-size:.78rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;display:flex;align-items:center;justify-content:space-between';marker.innerHTML='<span>✦ New Premium Tools</span><span style="color:#E8D28A">10 new utilities</span>';grid.prepend(marker)}
      return;
    }
  }catch(e){console.warn('Premium tool catalog fallback:',e)}
  const grid=document.getElementById('toolsGrid');if(!grid)return;
  const marker=document.createElement('div');marker.style.cssText='grid-column:1/-1;background:#0A0A0A;color:#fff;padding:14px 18px;font-size:.78rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase';marker.textContent='✦ New Premium Tools · 10 new utilities';grid.prepend(marker);
  tools.slice().reverse().forEach(function(t){const a=document.createElement('a');a.className='tool-card-item';a.href=t[3];a.innerHTML='<span class="tc-icon">'+t[0]+'</span><span class="tc-name">'+t[1]+'</span><span class="tc-desc">'+t[2]+'</span><span class="tc-arrow">Use Tool →</span>';grid.prepend(a)});
});